...
def simulate_enchant(trials: int, target: int, success_prob: float, processes: int = 25, batch_size: int = 1000) -> list[int]:
    if trials % 1000 != 0:
        raise ValueError("trials size must be multiple off 1000")
...
def main() -> None:
    ...
    plt.axhline(0.5, color='red', linestyle='dashed', label=f'Median: {median_val:.2f}')
    plt.axhline(0.9, color='orange', linestyle='dashed', label=f'90% ≤: {p90_val:.2f}')
    plt.axhline(0.99, color='green', linestyle='dashed', label=f'99% ≤: {p99_val:.2f}')
    plt.axhline(0.999, color='purple', linestyle='dashed', label=f'99.9% ≤: {p99_9_val:.2f}')
    plt.axvline(mean_val, color='blue', linestyle='dotted', label=f'Average: {mean_val:.2f}')
